...
# FIXED DATE INPUTS: No min/max, using unique keys to bypass browser cache errors
user_start = st.sidebar.date_input("Start Date", value=date(2020, 1, 1), key="bt_start_date")
user_end = st.sidebar.date_input("End Date", value=date.today(), key="bt_end_date")
...
if st.sidebar.button("🚀 Run Performance Audit"):
    # Auto-Correction for Date Range (Applied during execution)
    earliest_allowed = date.today() - timedelta(days=max_days_allowed)
    
    # Force start date to be within timeframe limits
    if user_start < earliest_allowed:
        final_start = earliest_allowed
        st.info(f"💡 Adjusted Start Date to {final_start} for {selected_tf_label} data limits.")
    else:
