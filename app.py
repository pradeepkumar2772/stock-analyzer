        ...
        if not data.empty:
            ...
            if trades:
                ...
                with t3:
                    ...
                    df_trades['day'] = df_trades['exit_date'].dt.day_name(); day_d = df_trades.assign(is_win=df_trades['pnl_pct'] > 0).groupby(['day', 'is_win']).size().unstack(fill_value=0); day_d.columns = ['Winners', 'Losers']; day_d = day_d.reindex(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'])
                    f4 = go.Figure(); 
