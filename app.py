...
def run_backtest(df, symbol, config, strategy_type, benchmark_df=None):
    ...
    # These are pre-calculated in the execution blocks below to ensure consistency
    ...
    for entry in ema_range:
        for ex in ema_range:
            ...
            if t:
                ...
                results.append({"Entry": entry, "Exit": ex, "Return %": round(f_ret, 2), "Max DD %": round(mdd, 2), "RF": round(f_ret / abs(mdd) if mdd != 0 else 0, 2)})
            if curr % 50 == 0: prog.progress(curr/steps)
    prog.empty()
    return pd.DataFrame(results)

# --- 4. SIDEBAR & UI ---
st.set_page_config(layout="wide", page_title="Pro-Tracer V3")
symbol = st.sidebar.text_input("Symbol", value="BRITANNIA.NS").upper()
strat_choice = st.sidebar.selectbox("Strategy", ["PK Strategy (Positional)", "RSI 60 Cross", "EMA Ribbon"])
...
config = {'ema_fast': 20, 'ema_slow': 50, 'ema_exit': 30, 'sl_val': 5.0, 'use_sl': True, 'use_slippage': True, 'slippage_val': 0.1}
...
if run_single or run_arena or run_opt:
    ...
    if not data.empty:
        ...
        # SHARED CALCULATION: Ensure Baseline PK EMAs are ALWAYS present
        data['ema_20_pk'] = data['close'].ewm(span=20, adjust=False).mean()
        data['ema_15_pk'] = data['close'].ewm(span=15, adjust=False).mean()
        ...
        if run_single:
            ...
            arena_results = []
            for s in ["PK Strategy (Positional)", "RSI 60 Cross", "EMA Ribbon"]:
