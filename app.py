...
# Enhanced Custom CSS for table transparency and colors
st.markdown("""
    <style>
    .stMetric { background-color: #1a1c24; padding: 15px; border-radius: 8px; border: 1px solid #2d2f3b; }
    .report-table { width: 100%; border-collapse: collapse; margin-top: 20px; font-family: sans-serif; font-size: 0.9rem; }
    .report-table th { background-color: #2d2f3b; color: #888; padding: 12px; border: 1px solid #2d2f3b; text-transform: uppercase; }
    .report-table td { border: 1px solid #2d2f3b; padding: 10px; text-align: center; color: #fff; }
    .profit { color: #2ecc71 !important; font-weight: bold; }
    .loss { color: #e74c3c !important; font-weight: bold; }
    .total-cell { background-color: #2d2f3b; font-weight: bold; color: #3498db !important; }
    </style>
...
# RESTORED RISK TOGGLES
use_sl = st.sidebar.toggle("Enable Stop Loss", value=True)
sl_val = st.sidebar.slider("Stop Loss %", 0.5, 15.0, 5.0) if use_sl else 0
use_tp = st.sidebar.toggle("Enable Target Profit", value=True)
tp_val = st.sidebar.slider("Target Profit %", 1.0, 100.0, 25.0) if use_tp else 0
            ...
            if trades:
                ...
                with t1:
                    ...
                    for year, row in pivot.iterrows():
                        html += f"<tr><td style='color:#888;'>{year}</td>"
                ...
                with t2:
                    ...
                    pc2.write(f"**Calmar Ratio:** {abs(cagr/mdd):.2f}" if mdd != 0 else "N/A")
