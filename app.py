...
def run_backtest(df, symbol, config):
    ...
    # Strategy: RSI Cross 60 Upward | Exit: RSI Cross 60 Downward
...
# --- 3. UI STYLING ---
st.set_page_config(layout="wide", page_title="Institutional RSI 60 Report")
st.markdown("""
    <style>
    ...
    try:
        ...
        if not data.empty:
            ...
            if trades:
                ...
                max_w_s = streaks[pnl_bool == 1].max() if not wins.empty else 0; max_l_s = streaks[pnl_bool == 0].max() if not losses.empty else 0
                ...
                with t1:
                    ...
                    st.divider()
                    df_exp = pd.DataFrame({"Parameter": ["Total Returns", "MDD", "Win Ratio", "Trades", "CAGR", "Sharpe"], "Value": [f"{total_ret:.2f}%", f"{mdd:.2f}%", f"{(len(wins)/len(df_trades)*100):.2f}%", len(df_trades), f"{cagr:.2f}%", f"{sharpe:.2f}"]})
                    st.download_button("📥 Download Quick Stats (CSV)", df_exp.to_csv(index=False).encode('utf-8'), f"{symbol}_Stats.csv", "text/csv")

                with t2:
                    cl, cr = st.columns([1, 2.5])
                    with cl:
                        with st.expander("📊 Backtest Details", expanded=True):
                            draw_stat("Scrip", symbol); draw_stat("Start Date", start_str); draw_stat("End Date", end_str); draw_stat("Duration", f"{duration.days // 365}Y, {duration.days % 365 // 30}M"); draw_stat("Segment", "NSE"); draw_stat("Timeframe", selected_tf)
                        ...
                        with st.expander("🔍 Trade Characteristics"):
                            draw_stat("Total Number of Trades", len(df_trades)); draw_stat("No. Profit Trades", len(wins)); draw_stat("No. Loss Trades", len(losses)); draw_stat("Max Profit", f"{df_trades['pnl_pct'].max()*100:.2f}"); draw_stat("Max Loss", f"{df_trades['pnl_pct'].min()*100:.2f}"); draw_stat("Winning Streak", f"{max_w_s}.00"); draw_stat("Lossing Streak", f"{max_l_s}.00")
                        with st.expander("🛡️ Risk-Adjusted Metrics"):
                            draw_stat("Sharpe Ratio", f"{sharpe:.2f}"); draw_stat("Calmar Ratio", f"{abs(cagr/mdd):.2f}")
                        with st.expander("⏱️ Holding Period"):
                            df_trades['hold'] = (df_trades['exit_date'] - df_trades['entry_date']).dt.days
                            draw_stat("Max Holding Period", f"{df_trades['hold'].max()} days"); draw_stat("Min Holding Period", f"{df_trades['hold'].min()} days"); draw_stat("Avg Holding Period", f"{df_trades['hold'].mean():.2f} days")
                ...
                with t3:
                    ...
                    st.plotly_chart(go.Figure(data=[go.Bar(x=y_r.index, y=y_r.values, text=y_r.values.round(1), texttemplate='%{text}%', textposition='outside', textfont=dict(color='white'), marker_color='#3498db')]).add_hline(y=0, line_color="white").update_layout(template="plotly_dark"), use_container_width=True)
                    ...
                    f2 = go.Figure()
                    for c, color in zip(['Total', 'Winners', 'Losers'], ['#3498db', '#2ecc71', '#e74c3c']): f2.add_trace(go.Bar(x=wl_y.index, y=wl_y[c], name=c, marker_color=color, text=wl_y[c], textposition='outside', textfont=dict(color='white')))
                    st.plotly_chart(f2.update_layout(barmode='group', template="plotly_dark"), use_container_width=True)
                    st.subheader("3. Exits Distribution")
                    ex_st = df_trades['exit_reason'].value_counts(normalize=True) * 100
                    st.plotly_chart(go.Figure(data=[go.Bar(x=ex_st.index, y=ex_st.values, text=ex_st.values.astype(int), texttemplate='%{text}%', textposition='outside', textfont=dict(color='white'), marker_color='#3498db')]).update_layout(template="plotly_dark"), use_container_width=True)
                    ...
                    f4 = go.Figure()
                    for c, color in zip(['Total', 'Winners', 'Losers'], ['#3498db', '#2ecc71', '#e74c3c']): f4.add_trace(go.Bar(x=day_d.index, y=day_d[c], name=c, marker_color=color, text=day_d[c], textposition='outside', textfont=dict(color='white')))
                    st.plotly_chart(f4.update_layout(barmode='group', template="plotly_dark"), use_container_width=True)
