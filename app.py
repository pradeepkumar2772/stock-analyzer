...
def run_backtest(df, symbol, config, strategy_type):
    ...
    # Pre-calculate Indicators (Manual Math as per Baseline)
    delta = df['close'].diff()
    ...
    elif strategy_type == "Trend-Momentum Synergy":
        # Book Logic: Buy when Trend is UP (EMA Cross) AND Momentum is UP (RSI > 60)
...
strat_choice = st.sidebar.selectbox("Select Strategy", ["RSI 60 Cross", "EMA Ribbon", "Trend-Momentum Synergy"])
