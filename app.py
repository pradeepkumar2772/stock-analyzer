...
def run_backtest(df, symbol, config):
    ...
    # Entry: RSI > 60 and Prev RSI <= 60 | Exit: RSI < 60 and Prev RSI >= 60
    ...
    try:
        ...
        if not data.empty:
            ...
            if trades:
                ...
                max_w_s = streaks[pnl_bool == 1].max() if not wins.empty else 0
                max_l_s = streaks[pnl_bool == 0].max() if not losses.empty else 0
                ...
                with t1:
                    ...
                    c9.metric("Risk-Reward Ratio", f"{(wins['pnl_pct'].mean()/abs(losses['pnl_pct'].mean())):.2f}" if not losses.empty else "N/A"); c10.metric("Expectancy", f"{expectancy:.2f}"); c11.metric("Sharpe Ratio", f"{sharpe:.2f}"); c12.metric("Calmer Ratio", f"{abs(cagr/mdd):.2f}" if mdd != 0 else "0.0")
                    st.divider()
                    ...
                    st.divider()
                    export_text = f"BACKTEST SUMMARY\nSymbol: {symbol}\nReturns: {total_ret:.2f}%\nMDD: {mdd:.2f}%\nWin Ratio: {(len(wins)/len(df_trades)*100):.2f}%"
                    st.download_button("📥 Download Report", export_text, f"{symbol}_report.txt")

                with t2:
                    cl, cr = st.columns([1, 2.5])
                    with cl:
                        with st.expander("📊 Backtest Details", expanded=True):
                            draw_stat("Scrip", symbol); draw_stat("Start Date", start_str); draw_stat("End Date", end_str); draw_stat("Duration", f"{duration.days // 365}Y, {duration.days % 365 // 30}M")
                        ...
                        with st.expander("🔍 Trade Characteristics"):
                            draw_stat("Total Trades", len(df_trades)); draw_stat("Profit Trades", len(wins)); draw_stat("Loss Trades", len(losses)); draw_stat("Max Profit", f"{df_trades['pnl_pct'].max()*100:.2f}"); draw_stat("Max Loss", f"{df_trades['pnl_pct'].min()*100:.2f}")
