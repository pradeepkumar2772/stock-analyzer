...
def run_backtest(df, symbol, config, strategy_type, benchmark_df=None):
    ...
    # (Original strategy elifs for Flags, Squeeze, Synergy, etc. remain here)
...
# --- 3. UI STYLING (IDENTICAL TO BASE) ---
            ...
            if trades:
                ...
                with t2:
                    ...
                    with cl:
                        ...
                        with st.expander("📈 Returns"): draw_stat("Total Ret", f"{total_ret:.2f}%"); draw_stat("Avg Ret/Trade", f"{df_trades['pnl_pct'].mean()*100:.2f}%")
                        # (Original 8 expander logic remains here)
                ...
                with t3:
                    ...
                    st.plotly_chart(go.Figure(data=[go.Bar(x=y_r.index, y=y_r.values, text=y_r.values.round(1), marker=dict(color='#3498db'))]).update_layout(title="Yearly Return (%)", template="plotly_dark"), use_container_width=True)
