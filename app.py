...
def run_backtest(df, symbol, config, strategy_type):
    ...
    # Logic for RSI and other baselines
    delta = df['close'].diff()
    ...
    try:
        ...
        if not data.empty:
            ...
            if trades:
                ...
                m1.metric("Total Return", f"{total_ret:.2f}%"); m2.metric("Win Ratio", f"{(len(wins)/len(df_trades)*100):.2f}%"); m3.metric("Trades", len(df_trades))
                
                t1, t2, t3, t4 = st.tabs(["Quick Stats", "Statistics", "Charts", "Trade Details"])
                with t1:
                    st.plotly_chart(px.line(df_trades, x='exit_date', y='equity', title="Growth of Capital"), use_container_width=True)
                with t2:
                    cl, cr = st.columns([1, 2.5])
                    with cl:
                        with st.expander("📊 Performance", expanded=True):
                            draw_stat("Total Ret", f"{total_ret:.2f}%")
                            draw_stat("Avg Ret/Trade", f"{df_trades['pnl_pct'].mean()*100:.2f}%")
                    with cr:
